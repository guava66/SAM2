...
class Hiera(nn.Module):
    ...
    #patch_depth 表示图像在深度方向（例如 3D 图像中的 z 轴）上划分的 patch 数量。即深度方向上有多少个 patch。
        ...
        for i in range(depth):
            ...
            if i < split_point:
                self.blocks_0.append(block)
            else:
                self.blocks_1.append(block)
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        B, C, H, W, D = x.shape
        x_reshaped = x.permute(0, 4, 1, 2, 3).reshape(B*D, C, H, W)
        # print(x.shape)
        #x.shape=torch.Size([512, 3, 512, 512])
        with torch.no_grad():
            x = self.patch_embed(x_reshaped)
            # print(x.shape)
            #x.shape=torch.Size([512, 32, 32, 768])
        if self.num_slice > 1:
            x = x.reshape(B, D, x.size(1), x.size(2), -1)
            x = x.permute(0, 4, 2, 3, 1)
            x = self.slice_embed(x)
            x = x.permute(0, 2, 3, 4, 1)
        else:
            x = x.reshape(B, D, x.size(1), x.size(2), -1)  # (B, D, P_H, P_W, EMB_DIM)
            x = x.permute(0, 2, 3, 1, 4)  # (B, P_H, P_W, D, EMB_DIM)
        ...
        for i, blk in enumerate(self.blocks_1):
            ...
            x = blk(x)
            if (i + len(self.blocks_0)==self.stage_ends[-1]) or ((i + len(self.blocks_0)) in self.stage_ends) and self.return_interm_layers:
                feats = x.permute(0, 4, 1, 2, 3)
                outputs.append(feats)
