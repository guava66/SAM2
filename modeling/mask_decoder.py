...
class MLAHead(nn.Module):
    ...
    def forward(self, mla_p2, mla_p3, mla_p4, mla_p5, scale_factor):
        # mla_p2 = mla_p2.to(self.device_0)
        # mla_p3 = mla_p3.to(self.device_0)
        # mla_p4 = mla_p4.to(self.device_0)
        # mla_p5 = mla_p5.to(self.device_0)
        ...
        head3 = F.interpolate(self.head3(
            mla_p3), scale_factor = scale_factor*2, mode='trilinear', align_corners=True)
        head4 = F.interpolate(self.head4(
            mla_p4), scale_factor = scale_factor*4, mode='trilinear', align_corners=True)
        head5 = F.interpolate(self.head5(
            mla_p5), scale_factor = scale_factor*8, mode='trilinear', align_corners=True)
...
class VIT_MLAHead(nn.Module):
    ...
    def __init__(self, device_0,img_size=768, mla_channels=256, mlahead_channels=128, num_classes=3,
        ...
        self.mlahead = MLAHead(mla_channels=self.mla_channels,
                               mlahead_channels=self.mlahead_channels, norm_cfg=self.norm_cfg,device_0=self.device_0)
